...
angular.module('app')
    ...
    .controller("PopularsController",
        ['$scope', '$http', '$filter', 'Notifications', 'cart', 'populars', 'Auth',
            function ($scope, $http, $filter, Notifications, cart, populars, $auth) {
                $scope.products = [];
                $scope.addToCart = function (item) {
                    cart.addToCart(item.product, parseInt(item.quantity)).then(function (data) {
                        Notifications.success("Added! Your total is " + $filter('currency')(data.cartTotal));
                ...
                // initialize products
                populars.getPopularProducts().then(function (data) {
                    $scope.products = data.map(function (el) {
                        return {
                            quantity: "1",
                            product: el
                    ...
                    Notifications.error("No products available: " + err.statusText);
                });
